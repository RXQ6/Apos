...
`passing | active | blocked | passing`  
...
| agent.workbench | 场景规划工作台 | P0 | passing | docs:verify | `features/agent.workbench/` |
| agent.provider_config | 模型通道配置 | P0 | passing | docs:verify | `features/agent.provider_config/` |
...
| customer.logout | 退出登录 | P1 | passing | docs:verify | `features/customer.logout/` |
| customer.address_save | 保存收货地址 | P1 | passing | docs:verify | `features/customer.address_save/` |
| catalog.search | 商品搜索 | P0 | passing | docs:verify | `features/catalog.search/` |
| catalog.detail | 商品详情 | P0 | passing | docs:verify | `features/catalog.detail/` |
| catalog.publish | 商品上架 | P1 | passing | docs:verify | `features/catalog.publish/` |
| catalog.off_shelf | 商品下架 | P1 | passing | docs:verify | `features/catalog.off_shelf/` |
| cart.add | 加购 | P0 | passing | docs:verify | `features/cart.add/` |
| cart.update | 改购物车 | P0 | passing | docs:verify | `features/cart.update/` |
| cart.checkout_ready | 结算就绪 | P0 | passing | docs:verify | `features/cart.checkout_ready/` |
| cart.merge | 登录合并购物车 | P0 | passing | docs:verify | `features/cart.merge/` |
...
| pricing.quote | 结算报价 | P0 | passing | docs:verify | `features/pricing.quote/` |
| pricing.coupon_receive | 领券 | P1 | passing | docs:verify | `features/pricing.coupon_receive/` |
| pricing.coupon_apply | 用券 | P1 | passing | docs:verify | `features/pricing.coupon_apply/` |
| pricing.promo_conflict | 优惠冲突 | P1 | passing | docs:verify | `features/pricing.promo_conflict/` |
...
| order.get | 订单详情 | P0 | passing | docs:verify | `features/order.get/` |
| order.repay | 再次支付 | P0 | passing | docs:verify | `features/order.repay/` |
...
| order.address_change | 支付前改址 | P1 | passing | docs:verify | `features/order.address_change/` |
...
| fulfillment.track | 物流轨迹 | P1 | passing | docs:verify | `features/fulfillment.track/` |
| fulfillment.partial_ship | 部分发货 | P1 | passing | docs:verify | `features/fulfillment.partial_ship/` |
| aftersale.approve | 售后审核 | P0 | passing | docs:verify | `features/aftersale.approve/` |
